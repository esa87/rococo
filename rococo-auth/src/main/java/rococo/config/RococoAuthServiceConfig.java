...
import rococo.config.keys.KeyManager;
import rococo.service.SpecificRequestDumperFilter;
...
public class RococoAuthServiceConfig {
  ...
  @Bean
  public CommandLineRunner checkProfiles(Environment env) {
    return args -> {
  ...
  @Bean
  public CommandLineRunner checkProperties(@Value("${rococo-front.base-uri}") String rococoFrontUri) {
    return args -> {
      System.out.println("rococo-front.base-uri: " + rococoFrontUri);
    };
  }
